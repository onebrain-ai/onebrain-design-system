...
│   ├── surface-mobile.html        SURFACE: iOS app flow (Vault · Capture · Skills) in iPhone frames
...
| [`preview/surface-mobile.html`](preview/surface-mobile.html) | **SURFACE.** iOS app flow (Vault · Capture · Skills) in inline iPhone frames — 44px tab bars, working `/capture` composer with `[[wikilink]]` chips | `--section-accent`→magenta, `--font-mono`, form patterns, glow tokens |
